...
function push_ndock_list(req) {
	...
	for(var id in $ship_list) {
		var ship = $ship_list[id];
		if (ship.locked && !$ndock_list[id] && ship.nowhp < ship.maxhp) {
			var r = ship.nowhp / ship.maxhp;
	...
	if (api_name == '/api_start2/getData') {
		...
		func = function(json) { // 保有艦、艦隊一覧を更新してcond表示する.
			...
			if (decode_postdata_params(request.request.postData.params).api_shipid)
				delta_update_ship_list(d.api_ship_data); // 装備解除時は差分のみ.
			else
		...
		func = function(json) { // 保有艦、艦隊一覧を更新してcond表示する.
			...
			delta_update_ship_list([
				d.api_ship_data.api_set_ship,
				d.api_ship_data.api_unset_ship
			]);
			print_port();
		};
	}
		...
		func = function(json) { // 保有艦、艦隊一覧を更新してcond表示する.
			let d = json.api_data;
			delta_update_ship_list([d.api_ship_data]); // 2019.1.22メンテ更新で, slotからshipに変わった.
			print_port();
		};
	}
