...
function calc_kouku_damage(result, title, kouku, hp, hc) {
	...
	calc_damage(result, title, kouku.api_stage3, hp);				// 航空爆撃雷撃戦.
	calc_damage(result, title, kouku.api_stage3_combined, hp, hc);	// 連合第二艦隊：航空爆撃雷撃戦.
}
...
=======
		var t0 = d.api_flare_pos[0]; if (t0 != -1) result.detail.push({ty:'照明弾(夜戦)', at: nowhps_c && maxhps_c[1] > 0 ? -t0 : t0});
>>>>>>> origin/night-battle-flare-fix
	...
	for (var i = 1; i <= 12; ++i) {
		...
		if (i > 6)
			req.push('\t' + i + '(' + name + ').\t'
				+ hp_status_on_battle(nowhps_c[i], maxhps_c[i], beginhps_c[i]) + '\t');
		else
			req.push('\t' + i + '(' + name + ').\t'
				+ hp_status_on_battle(nowhps[i+6], maxhps[i+6], beginhps[i+6]) + '\t');

		var msg = ['YPS_enemy_detail' + i];
		var enemy_slot = eSlot[i-1];
		var param_name = ['火力', '雷装', '対空', '装甲'];
		var enemy_param = eParam[i-1];
		var enemy_kyouka = eKyouka ? eKyouka[i-1] : [0,0,0,0];
		var param = [];
		for(var j = 0; j < 4; ++j){
