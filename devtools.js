...
function push_all_fleets(req) {
	for (var f_id in $fdeck_list) {
		...
		if (mission_end > 0) {
			...
			req.push('遠征' + id + ' ' + $mst_mission[id].api_name + ': ' + d.toLocaleString() + '(残' + msec_name(ms) +  ')');
...
//			if (dt.damage && dt.target) dt.damage = hp_status_on_battle(dt.hp, (dt.target >= 20 ? maxhps_c[dt.target-20] : maxhps[dt.target]), dt.hp + dt.damage);
